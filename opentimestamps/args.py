...
from opentimestamps import cmds, cache
...
def make_common_options_arg_parser():
    ...
    parser.add_argument('--version', action='version', version='v%s' % opentimestamps.__version__)
...
def handle_common_options(args, parser):
    ...
    args.cache = cache.TimestampCache(args.cache_path)
...
def parse_ots_args(raw_args):
    ...
    parser_stamp.set_defaults(cmd_func=cmds.stamp_command)
    parser_upgrade.set_defaults(cmd_func=cmds.upgrade_command)
    parser_verify.set_defaults(cmd_func=cmds.verify_command)
    parser_info.set_defaults(cmd_func=cmds.info_command)
